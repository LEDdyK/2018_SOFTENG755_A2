...
def naiveBayesGaussian(entries, features_avail, target):
...
def naiveBayesCV(features, entries, target, user_in):
    if (user_in == 'occupancy'):
        features_sel = features[['Light', 'Humidity', 'HumidityRatio']].copy()
    else:
        features_sel = features[[17, 12, 19, 27]].copy()
    
    features_prep = fp.prepareFeatures(features_sel, entries)
    classifier = GaussianNB()
    scores = cross_val_score(classifier, features_prep, target, cv = 10, scoring = 'accuracy')
    return scores
