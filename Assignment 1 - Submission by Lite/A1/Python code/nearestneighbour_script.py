...
def nearestNeighbour(entries, features_avail, target):
...
def nearestNeighbourOpt(features, entries, target, user_in):
    if (user_in == 'occupancy'):
        features_sel = features[['Light', 'Humidity', 'Temperature', 'HumidityRatio']].copy()
    else:
        features_sel = features[[17, 16, 19, 22, 2, 15, 13, 0]].copy()
    
    features_prep = fp.prepareFeatures(features_sel, entries)
    
    classifier = KNeighborsClassifier()
    nrange = list(range(1, 51))
...
def drawPlot(features, entries, target, user_in):
    if (user_in == 'occupancy'):
        features_sel = features[['Light', 'Humidity', 'Temperature', 'HumidityRatio']].copy()
        classifier = KNeighborsClassifier(n_neighbors = 49, weights = 'uniform')
    else:
        features_sel = features[[17, 16, 19, 22, 2, 15, 13, 0]].copy()
